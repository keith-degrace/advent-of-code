...
const getShortestPath = (grids: Grid[], start: Position, end: Position, time: number): number => {
    const open: Node[] = [{ position: { ...start, time }, f: 0, h: getManhattanDistance(start, end), g: 0 }];
    const closed: Node[] = [];
...
const main = () => {
    ...
    time += getShortestPath(grids, start, end, time);
    log(`  First leg done`);

    time += getShortestPath(grids, end, start, time);
    log(`  Second leg done`);
