...
func hasValidGroupTwoThreeAndFour(inputs2 []int, targetSum int) bool {
	for j := 1; j < len(inputs2) - 1; j++ {
		...
		for _,group2 := range getGroups(inputs2, group2Size, targetSum) {
			...
			for k := 1; k < len(inputs3) - 1; k++ {
				group3Size := k
		
				for _,group3 := range getGroups(inputs3, k, targetSum) {
					inputs4 := getSubInput(inputs3, group3)
					group4Size := len(inputs3) - group3Size

					if hasGroup(inputs4, group4Size, targetSum) {
						return true
					}
				}
			}
...
func getBestQuantumEntanglement(inputs []int, targetSum int) int {
	...
	for i := 1; i < len(inputs); i++ {
		...
		for _,group1 := range getGroups(inputs, group1Size, targetSum) {
			...
			if hasValidGroupTwoThreeAndFour(inputs2, targetSum) {
				bestQE = qe
			}
		}
