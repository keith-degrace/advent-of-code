...
func run(instructions []Instruction, ipRegister int, register Register) {
	...
	for ip < len(instructions) {
		...
		// The exit condition is at ip 28 where it will kick out if r0 == r2.  Since R0 is never modified by the program, we can simply
		// find the first value of r2 that will be compared to r0.  If we feed this into r0, the program will kick out.
		if ip == 28 {
