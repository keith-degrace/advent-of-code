...
func parseInputs(inputs []string, elfAttackPower int) (CaveMap, []Player) {
	caveMap := CaveMap{}
	...
	for y,input := range inputs {
		for x,char := range input {
			if char == '#' {
				...
				players = append(players, Player{nextPlayerId, 'E', Coord { x, y }, 200, elfAttackPower })
				nextPlayerId++
			}
		}
	}
...
func getPlayerCount(players []Player, kind byte) int {
	count := 0
...
func iterate(caveMap CaveMap, players []Player) ([]Player, bool) {
	sort.Sort(byPlayer(players))
	
	for i := 0; i < len(players); i++ {
		if getPlayerCount(players, 'E') == 0 || getPlayerCount(players, 'G') == 0 {
...
func main() {
	...
	for elfAttackPower := 4; ; elfAttackPower++ {
		caveMap, players := parseInputs(inputs, elfAttackPower)
		originalElfCount := getPlayerCount(players, 'E')

		round := 1
		for {
			var full bool
			players, full = iterate(caveMap, players)

			elfCount := getPlayerCount(players, 'E')
			goblinCount := getPlayerCount(players, 'G')

			if elfCount < originalElfCount {
				break;
			}

			if goblinCount == 0 {
				fmt.Println("Round", round)
				fmt.Println("Attack Power", elfAttackPower)
				fmt.Println("Hit Point Sum", getHitPointSum(players))
				printPlayerHP(players)

				if full {
					fmt.Println(round * getHitPointSum(players))
				} else {
