...
func parseInputs(inputs []string, boost int) Groups {
	groups := Groups{}
...
func hasWinner(groups Groups) (bool, bool) {
	...
	return !hasImmuneSystemGroup || !hasInfectionGroup, hasImmuneSystemGroup
}
...
func main() {
	...
	boost := 0
	for {
		groups := parseInputs(inputs, boost)
		...
		for {
			...
			hasWinner, isImmuneSystem := hasWinner(groups)
			if hasWinner {
				if isImmuneSystem {
