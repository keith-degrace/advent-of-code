...
export const setPixel = <T>(grid: Grid<T>, position: Position, value: T): void => {
    ...
    grid.width = grid.max.x - grid.min.x + 1;
    grid.height = grid.max.y - grid.min.y + 1;
};
...
export const printGrid = <T>(grid: Grid<T>, emptyChar = "."): void => {
    for (let y = grid.min.y; y <= grid.max.y; y++) {
        let line = "";

        for (let x = grid.min.x; x <= grid.max.x; x++) {
            line += getPixel(grid, { x, y }) ?? emptyChar;
        }
...
export const countValueInGrid = <T>(grid: Grid<T>, value: T): number => {
    let count = 0;
