...
func move(instruction Instruction, input string) string {
	letter := input[instruction.number1]

	if instruction.number1 < instruction.number2 {
		input = deleteChar(input, instruction.number1)
		input = insertStringChar(input, instruction.number2, letter)
	} else {
		input = insertStringChar(input, instruction.number2, letter)
		input = deleteChar(input, instruction.number1+1)
	}
...
func Solve() {
	...
	inputs, value := au.ReadInputAsStringArray("21"), "abcdefgh"
	// inputs, value  = testInputs(), "decab"

	instructions := parseInputs(inputs)

	for _, instruction := range instructions {
		value = apply(instruction, value)
		fmt.Println(value)
	}
