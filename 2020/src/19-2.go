...
type Puzzle19_2 struct {
}

func (p Puzzle19_2) parse(input []string) (map[int]Rule19_2, []string) {
	lineIndex := 0

	charRules := make(map[int]*CharRule19_2)
	sequenceRules := make(map[int]*SequenceRule19_2)
	...
	for _, sequenceRule := range sequenceRules {
		sequenceRule.ruleSets = [][]Rule19_2{}
...
type Rule19_2 interface {
	getId() int
	matches(text string) bool
}

type CharRule19_2 struct {
	id   int
	char byte
}

func (r *CharRule19_2) getId() int {
	return r.id
}

func (r *CharRule19_2) matches(text string) bool {
	return len(text) == 1 && text[0] == r.char
}

type SequenceRule19_2 struct {
	id           int
	ruleSets     [][]Rule19_2
	minMatchSize int
...
func (r *SequenceRule19_2) getId() int {
	return r.id
}

func (r *SequenceRule19_2) ruleSetMatches(ruleSet []Rule19_2, text string) bool {
...
func (r *SequenceRule19_2) matches(text string) bool {
...
func (p Puzzle19_2) run() {
	input := au.ReadInputAsStringArray("19")

	for i := range input {
		if strings.HasPrefix(input[i], "8:") {
			input[i] = "8: 42 | 42 8"
		}

		if strings.HasPrefix(input[i], "11:") {
